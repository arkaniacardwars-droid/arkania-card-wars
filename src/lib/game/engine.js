 ...
 *
 * ESM puro — roda no browser, no SSR (SvelteKit) e no Node (vitest).
 */

export const ATTRS = ['ATA', 'DEF', 'VEL', 'ENE', 'VID'];
...
export function energiaMax(j) {
	return j.carta.attrs.ENE;
}
export function podeEspecial(j) {
	return j.energia >= CONST.CUSTO_ESPECIAL;
}

export function criarJogador(carta, nome) {
	return {
		nome: nome || 'Duelista',
		carta: carta,
...
export function criarEstado(cartaA, cartaB, nomeA, nomeB, desempate) {
	var A = criarJogador(cartaA, nomeA || 'Você');
	var B = criarJogador(cartaB, nomeB || 'Adversário');
	var va = A.carta.attrs.VEL,
		vb = B.carta.attrs.VEL;
 ...
 */
export function aplicarTroca(estado, jogadaAtk, jogadaDef) {
	var atkKey = estado.atacante,
		defKey = atkKey === 'A' ? 'B' : 'A';
	var atk = estado[atkKey],
		def = estado[defKey];
	...
	var poder = tipo === 'especial' ? atk.carta.attrs.ATA * CONST.ESP_MULT : atk.carta.attrs.ATA;
	var dano = Math.max(
		CONST.DANO_MIN,
